...
function loadStatistical(sdata){
  ...
  var loadMoreBtn = `
    ...
    <div id="cf-footer" class="cf-new-add">
     ...
     <span class="cf-data-lastupdated">更新于：${escapeFcircleHtml(sdata.last_updated_time)}</span>
     <span class="cf-data-lastupdated">订阅:${escapeFcircleHtml(sdata.friends_num)} 活跃:${escapeFcircleHtml(sdata.active_num)} 日志:${escapeFcircleHtml(sdata.article_num)}</span>
    </div>
...
function fetchNextArticle(){
  ...
  if(start <  articleNum){
    ...
    fetch(fetchUrl)
      .then(res => res.json())
      .then(json =>{
        var nextArticle = parseArticleData(json.article_data);
